...
Class Database extends PaniCode_{
	...
	{
		echo PHP_EOL;
		self::$ActionList = ['Create connection','Export database','Import database','Create database'];
		$n=1;
	...
	public function SubTakeAction()
	{
		...
		}elseif($a=='2')
		{
			...
			$dir = $dbname.'-panicode.sql';
			echo "[!] Backing up database to {$dir} ...";
			#require 'commander/0-php_server.php';
			self::console("mysqldump --user={$user} --password={$pass} --host={$host} {$dbname} --result-file={$dir} 2>&1");
		}elseif($a=='3')
		{
			echo "Hostname   ::";$host=trim(fgets(STDIN));
			echo "Username   ::";$user=trim(fgets(STDIN));
			...
			#require 'commander/0-php_server.php';
			self::console("mysql -h ${host} -u {$user} -p{$pass} -d {$dbname} < {$sq} ");
		}elseif($a=='4')
		{
			echo "Hostname   ::";$host=trim(fgets(STDIN));
			echo "Username   ::";$user=trim(fgets(STDIN));
			...
			self::console("mysql -h ${host} -u ${user} -p{$pass} -e 'CREATE DATABASE {$dbname}'; ");
		}
	}
}
